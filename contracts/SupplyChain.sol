...
contract SupplyChain {
  ...
  */
  ...
  /* Create 4 events with the same name as each possible State (see above)
    ...
    event LogSold(uint indexed sku, address indexed buyer);
...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier paidEnough(uint _skuCount, uint _price) {
  ...
  modifier checkValue(uint _skuCount) {
    ...
    items[_skuCount].buyer.transfer(amountToRefund);
  }
  ...
  modifier forSale(uint _skuCount) {
    require(items[_skuCount].state == State.ForSale, "The item's state must be ForSale!");
    require(items[_skuCount].seller != address(0), "failing");
    _;
  }

  modifier sold(uint _skuCount)
  {
    require(items[_skuCount].state == State.Sold);
    _;
  }

  modifier shipped(uint _skuCount)
  {
    require(items[_skuCount].state == State.Shipped);
    _;
  }
  ...
  modifier received(uint _skuCount)
  {
    ...
    require(items[_skuCount].state == State.Received);
    _;
  }
  ...
  function buyItem(uint _sku)
    ...
    forSale(_sku)
    checkValue(_sku)
    paidEnough(_sku, msg.value)
  {
    items[_sku].buyer = msg.sender;
    items[_sku].state = State.Sold;
    uint256 itemPrice = items[_sku].price;
    items[_sku].seller.transfer(itemPrice);
    emit LogSold(skuCount, msg.sender);
  }
